...
export const SelectChain: React.FC<{
  ...
  return (
    <div className="flex items-center justify-between bg-[rgba(85,123,241,0.02)] rounded-[8px] cursor-pointer min-w-[170px] relative">
      ...
      <div 
        ref={node}
        style={{ height: showOptions ? 'auto' : 0, padding: showOptions ? '8px 0 8px 0' : '0' }}
        className={`options absolute left-0 right-0 top-[110%] z-50 bg-white overflow-hidden rounded-lg transition-all `}>
          <div className=" max-h-[140px] overflow-auto scrollbar-container">
          {
            options.map(item => {
              return <div
                key={item.value} 
